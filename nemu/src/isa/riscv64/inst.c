...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi, I, R(dest) = src1 + src2);
  INSTPAT("000000? ????? ????? 001 ????? 00100 11", slli, I, R(dest) = U64(src1) << src2);
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 01110 11", sllw, R, R(dest) = SEXT(U64(src1) << BITS(src2, 4, 0), 32));
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltiu, R, if (src1 < U64(src2)) R(dest) = 1);
  /*add by leesum*/

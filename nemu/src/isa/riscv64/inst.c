...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bgeu, B, if (U64(src1) >= U64(src2)) s->dnpc = s->pc + dest);
  INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi, I, R(dest) = src1 & src2);
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi, I, R(dest) = src1 + src2);
  INSTPAT("000000? ????? ????? 001 ????? 00100 11", slli, I, R(dest) = U64(src1) << src2);
  INSTPAT("000000? ????? ????? 001 ????? 00110 11", slliw, I, R(dest) = SEXT(U64(src1) << src2, 32));
  INSTPAT("000000? ????? ????? 101 ????? 00110 11", srliw, I, R(dest) = SEXT(U32(src1) >> src2, 32));
  INSTPAT("000000? ????? ????? 101 ????? 00100 11", srli, I, R(dest) = U64(src1) >> src2);
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw, R, R(dest) = S32(src1) >> BITS(src2, 4, 0));
  ...
  INSTPAT("010000? ????? ????? 101 ????? 00110 11", sraiw, I, R(dest) = SEXT(S32(src1) >> src2, 32));

...
Devicevga::~Devicevga() { delete vgaregs.fbbuff; }
...
void Devicevga::write(paddr_t addr, word_t data, uint32_t len) {
  paddr_t offset;
  /* 控制寄存器 */
  if (atRange(deviceinfo.at(0).addr,
              deviceinfo.at(0).addr + deviceinfo.at(0).len - 1, addr)) {
  ...
  /* fb 缓存 */
  else if (atRange(deviceinfo.at(1).addr,
                   deviceinfo.at(1).addr + deviceinfo.at(1).len - 1, addr)) {
...
uint32_t Devicevga::screen_width() { return SCREEN_W; }

uint32_t Devicevga::screen_height() { return SCREEN_H; }
